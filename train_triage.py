...
postfix = "-train-triage-E%d"%(args.epochs)
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        if row['ID'] in ['0018','0147','0193','0119','0259','0274','0188','0058','0036','0078','0283','0239']:
            continue
        if (tri_tmp=='0') & (gt_tmp=='0'):
            eq.append(row['ID'])
        if (tri_tmp=='1') & (gt_tmp=='1'):
            eq.append(row['ID'])
        if (tri_tmp=='0') & (gt_tmp=='1'):
            ne.append(row['ID'])
        if (tri_tmp=='1') & (gt_tmp=='0'):
            ne.append(row['ID'])
    
    totallist = []
    totallist.append([j for j in eq if int(j) < 196])
    totallist.append([j for j in eq if (int(j) >= 196 and int(j) < 237)])
    totallist.append([j for j in eq if int(j) >= 237 and int(j) < 296])
    totallist.append([j for j in ne if int(j) < 196])
    totallist.append([j for j in ne if (int(j) >= 196 and int(j) < 237)])
    totallist.append([j for j in ne if int(j) >= 237 and int(j) < 296])
