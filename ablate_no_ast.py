...
import dataset.load_S4_MedIA_ADV as dataload
import model.S4_MedIA_ADV as my_models
...
postfix = "-ablate-no-ast-32-E%d-AdamW-0296-E4"%(args.epochs)
...
def main():
    ...
    train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset = dataload.LoadData(arg_root, 
                                                                             spec_path, 0, totallist,
                                                                             arg_batchsize_train,
                                                                             arg_batchsize_eval)
    args.class_num = NUM_CLASS
    ResNet_parameterDir = 'model/fair_7.pt'
    model = my_models.S4_MedIA(video_model_pth=ResNet_parameterDir, n_layers=3, d_model=3072,d_input=1024,d_output=2,dropout=0.4,l_max=64)
    model.encoder.audio_model = torch.load('model/resnet18_aud.pth')
    dis = my_models.Discriminator(n_downsampling=2, ndf=24)
...
def train(gradient_scaler, train_loader, model, dis, criterion1, criterion2, optimizer1, optimizer2, epoch):
    ...
    model.zero_grad()
    dis.zero_grad()
    for i, (input_first, spec_img, target_first, index, input_adv, spec_adv, target_adv) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            # else:
            pred_score, pred_logit, fea = model(imgvar, specvar, phrase='train')
            fea_adv = model(input_adv, spec_adv, phrase='adv')
...
def validate(val_loader, model, val_dataset):
    ...
    index_vector = []
    with torch.no_grad():
        for i, (input_first, spec_img, target_first, index) in enumerate(val_loader):
            target = target_first.cuda()
            imgvar = input_first.cuda()  # input: [16,3, 128,128] target:[16]
            specvar = spec_img.cuda()
            # compute output
            ''' model & full_model'''
            pred_score = model(imgvar, specvar, phrase='eval')
