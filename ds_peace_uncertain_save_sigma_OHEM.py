...
postfix = "-NEW-S4-MedIA-PEACE-UNCERTAIN-OHEM-Save-Sigma-FULL-LR-4-AFTER-4-w%.2f-E%d-weight-%s"% (args.wi, args.epochs, args.w)  #L200 means 200*2 frames, I original use 400*2 frames
...
class UncertaintyLossWithOHEM(nn.Module):
    ...
    def forward(self, y_hat, y_mri, y_triage, sigma):
        ...
        uncertainty_loss = ce_loss / (2 * sigma.squeeze()**2) + w * torch.log(sigma.squeeze())
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print("=====Validating=====")
        prec1, predtmp, predscore, targ, result_names , sigma_val= validate(val_loader, model, val_dataset)
        ...
        _, _, predscore_test, targ_test, _, _ = validate(test_loader, model, test_dataset)
...
def validate(val_loader, model, val_dataset):
    ...
    with torch.no_grad():
        for i, (input_first, spec_img, audio_first, target_first, triage_first, index, filename) in enumerate(val_loader):
            ...
            sigma_vector.append([sigma.detach().cpu().numpy(), filename, target_first.detach().cpu().numpy(), triage_first.detach().cpu().numpy()]) 
