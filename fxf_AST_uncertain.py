...
NUM_EPOCH = 30
LEANRING_RATE = 1e-4 #last one: 1e-3
CV_FOLD = 5
LAMBDA = 5
# BATCH_SIZE_TRAIN = 64
# BATCH_SIZE_TEST = 64
...
postfix = "-fxf-AST-UNCERTAIN-E%d-ADAMW-1E-4"%(args.epochs)
...
def main():
    ...
    train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset = dataload.LoadData(arg_root, audio_root,     spec_path, 0, totallist, tri_dic, arg_batchsize_train,
                                                                             arg_batchsize_eval)
    ...
    criterion1 = UncertaintyLoss(alpha=1.0, lambda_weight = 0.1) # nn.CrossEntropyLoss(weight=torch.tensor(wi)).cuda()
