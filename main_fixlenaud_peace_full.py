...
import dataset.load_S4_MedIA_PEACE_ADV_SEG as dataload
...
postfix = "-final-S4-Peace-LR%d-E%d-ADAMW-w%.2f-full"%(args.lr_ratio,args.epochs, args.wi)
...
def main():
    ...
    arg_root =  '../stroke_data/video_segment_fix/'   # '/Feature/Frames256/', 
    ...
    train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset = dataload.LoadData(arg_root, audio_root,  spec_path, 0, totallist, arg_batchsize_train, arg_batchsize_eval) 
    ...
    criterion1 = nn.CrossEntropyLoss(weight=torch.tensor([args.wi, 1.0])).cuda()
...
def validate(val_loader, model, val_dataset):
    ...
    with torch.no_grad():
        for i, (input_first, spec_img, audio_first, target_first, index) in enumerate(val_loader):
            ...
            # compute output
            ''' model & full_model'''
            pred_score, _ = model(imgvar, specvar, astvar, phrase='eval')
