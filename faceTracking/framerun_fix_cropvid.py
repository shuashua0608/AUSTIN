...
def main():
    file_path = "../Raw_Video/"+args["Videopath"]+".MOV"
    # if not os.path.isdir('../../Stroke_Net/frames/'+args["Videopath"]):
    #     return
    # print("==================Initializing Face Tracking================")
    # if not os.path.isdir('../vid_fix/'+args["Videopath"]):
    #     os.mkdir('../vid_fix/'+args["Videopath"])
    ...
    out = cv2.VideoWriter('../vid_fix_560_200/'+args["Videopath"]+'.avi', fourcc, 30.0, (560,560))
    ...
    while success:
        ...
        x1=max(0,int(y-200))
        x2=min(int(y+h+200),frame.shape[0])
        y1=max(0,int(x-200))
        y2=min(int((x+w)+200),frame.shape[1])
        out_frame = frame[x1:x2,y1:y2]


        canvas = 225*np.ones((int(h)+401,int(h)+401,3), np.uint8)
        canvas[max(0,-int(y-200)):(max(0,-int(y-200))+out_frame.shape[0]),max(0,-int(x-200)):(max(0,-int(x-200))+out_frame.shape[1])] = out_frame
        pts1 = np.float32([[0,0],[0,canvas.shape[0]],[canvas.shape[0],canvas.shape[0]],[canvas.shape[0],0]])
        pts2 = np.float32([[0,0],[0,560],[560,560],[560,0]])

        M = cv2.getPerspectiveTransform(pts1,pts2)

        dst = cv2.warpPerspective(canvas,M,(560,560))
        ...
        #     break
        # if fn > 6:
        # cv2.imwrite('tmp/'+str(fn)+'.jpg',dst)
        # return
