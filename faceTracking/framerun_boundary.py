...
def rotate_image(image, angle):
  ...
  result = cv2.warpAffine(image, rot_mat, image.shape[1::-1], flags=cv2.INTER_LINEAR,borderValue=(225, 225, 225))
  return result


def main():
    file_path = "../../Stroke/Raw_Video/"+args["Videopath"]+".MOV"
    if not os.path.isdir('../../Stroke_Net/frames/'+args["Videopath"]):
        return
    # print("==================Initializing Face Tracking================")
    if not os.path.isdir('../Feature/Frames_boundary/'+args["Videopath"]):
        os.mkdir('../Feature/Frames_boundary/'+args["Videopath"])
    
    if len(os.listdir('../Feature/Frames_boundary/'+args["Videopath"])) > 400:
        return
    print(file_path)
    ...
    while success:
        ...
        x1=max(0,int(y/2-100))
        x2=min(int((y+h)/2+100),frame.shape[0])
        y1=max(0,int(x/2-100))
        y2=min(int((x+w)/2+100),frame.shape[1])
        out_frame = frame[x1:x2,y1:y2]


        canvas = 225*np.ones((int(h/2)+201,int(h/2)+201,3), np.uint8)
        canvas[max(0,-int(y/2-100)):(max(0,-int(y/2-100))+out_frame.shape[0]),max(0,-int(x/2-100)):(max(0,-int(x/2-100))+out_frame.shape[1])] = out_frame
        pts1 = np.float32([[0,0],[0,canvas.shape[0]],[canvas.shape[0],canvas.shape[0]],[canvas.shape[0],0]])
        pts2 = np.float32([[0,0],[0,280],[280,280],[280,0]])

        M = cv2.getPerspectiveTransform(pts1,pts2)

        dst = cv2.warpPerspective(canvas,M,(280,280))
        ...
        if not os.path.isfile('../../Stroke_Net/frames/'+args["Videopath"]+'/'+str(fn)+'.jpg'):
            success,frame = videoCapture.read()
            continue
        ...
        # dst = dst[12:268,12:268]
        # cv2.imshow("face",dst)
        # key = cv2.waitKey(1) & 0xFF
        # if key == ord("q"):
        #     break
        cv2.imwrite('../Feature/Frames_boundary/'+args["Videopath"]+'/'+str(fn)+'.jpg',dst)
        # prev_frame = frame
