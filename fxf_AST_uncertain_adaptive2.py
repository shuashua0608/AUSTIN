...
postfix = "-fxf-AST-UNCERTAIN-E%d-ADAMW-1E-3-adaptive2"%(args.epochs)
...
#wi = torch.tensor([2.0, 1.0], dtype=torch.float32)  
...
def main():
    ...
    criterion1 = AdaptiveUncertaintyLoss(max_epochs=50) # nn.CrossEntropyLoss(weight=torch.tensor(wi)).cuda()
    ...
    for epoch in range(args.epochs):
        ...
        skip_lr_sched = train(gradient_scaler, train_loader,train_dataset, model, dis, criterion1, criterion2, optimizer, optimizer_adv, epoch)
        print("=====Validating=====")
...
def train(gradient_scaler, train_loader, train_dataset, model, dis, criterion1, criterion2, optimizer1, optimizer2, epoch):
    ...
    for i, (input_first, spec_img, fbank_cookie, target_first, triage_first, index, input_adv, spec_adv, fbank_adv, target_adv) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            sigma_vector.append([sigma, target_var, triage_var])
