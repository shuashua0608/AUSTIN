...
postfix = "-NEW-fxf-AST-UNCERTAIN-Adaptive-E%d-ADAMW-1E-3-weight-%s-softplus"%(args.epochs, args.wi)
#postfix = 'test_adaptive_loss'
...
class AdaptiveUncertaintyLoss(nn.Module):
    def __init__(self, initial_alpha=1.0, final_alpha=0.1, 
                 initial_epsilon=1.0, final_epsilon=0.1, curriculum_epochs=50):
        super().__init__()
    ...
    def forward(self, y_hat, y_mri, y_triage, sigma):
        ...
        curriculum_factor = max(0, 1 - self.current_epoch / self.curriculum_epochs)
        
        # Interpolate alpha and epsilon
        alpha = self.initial_alpha + progress * (self.final_alpha - self.initial_alpha)
        epsilon = self.initial_epsilon + progress * (self.final_epsilon - self.initial_epsilon)
        ...
        w = torch.exp(-alpha * torch.abs(y_mri - y_triage))
        base_loss = ce_loss / (2 * sigma**2) + w * torch.log(sigma)
        
        # Apply curriculum weighting to focus on easier samples first
        uncertainty_weight = torch.exp(-curriculum_factor * sigma)
        loss = base_loss * uncertainty_weight
        
        return loss.mean()

# class AdaptiveUncertaintyLoss(nn.Module):
#     def __init__(self, alpha = 1.0, epsilon=1e-6, beta = 0.1, max_epochs=50):
#         super().__init__()
...
#         """
#         # Ensure sigma is positive and (?) log(sigma) is also positive
#         sigma = torch.abs(sigma) + self.epsilon

#         # Early epochs: Reduce sigma's impact to focus on confident samples
#         # Later epochs: Let the model learn from harder cases
#         epoch_factor = (1 - self.current_epoch / self.max_epochs)  # Decay over time
#         adaptive_sigma = sigma * (1 + self.beta * epoch_factor)  
        
#         w = torch.exp(-self.alpha * torch.abs(y_mri - y_triage))
        
#         criterion = nn.CrossEntropyLoss().cuda() #weight=torch.tensor(wi)
#         ce_loss = criterion(y_hat, y_mri) #.cuda()
#         # Uncertainty loss component
#         loss = ce_loss / (2 * adaptive_sigma**2) + w * torch.log(adaptive_sigma)

#         return loss.mean()
        
#     def update_epoch(self, epoch):
#         """ Call this method at the beginning of each epoch to update the training schedule. """
#         self.current_epoch = epoch
...
def main():
    ...
    criterion1 = AdaptiveUncertaintyLoss() 
