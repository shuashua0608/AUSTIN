...
import model.yu_complex_operations as cops
...
def cauchy_conj(v, z, w, num=2, denom=2):
    ...
    return view_as_complex(r)

#_conj = lambda x: torch.cat([x, x.conj()], dim=-1)

# def _conj (x):
#     real_a, imag_a = torch.real(x), torch.imag(x)
#     real_b, imag_b = torch.real(x.conj()), torch.imag(x.conj())
#     real_result = torch.cat([real_a, real_b], dim=-1)
#     imag_result = torch.cat([imag_a, imag_b], dim=-1)
#     return torch.complex(real_result, imag_result)

def _conj (x):
    real_a, imag_a = torch.real(x), torch.imag(x)
    real_b, imag_b = torch.real(cops.complex_conj(x)), torch.imag(cops.complex_conj(x))
    real_result = torch.cat([real_a, real_b], dim=-1)
    imag_result = torch.cat([imag_a, imag_b], dim=-1)
    return torch.complex(real_result, imag_result)
...
def cauchy_naive(v, z, w):
    ...
    v = _conj(v)
    w = _conj(w)
    vv = cops.complex_unsqueeze(v,-1)
    ww = cops.complex_unsqueeze(w,-1)
    zz = cops.complex_unsqueeze(z,-2)

    zw = cops.complex_minus (zz, ww)
    
    #cauchy_matrix = v.unsqueeze(-1) / (z.unsqueeze(-2) - w.unsqueeze(-1)) # (... N L)
    cauchy_matrix = cops.complex_division(vv, zw)
    #return torch.sum(cauchy_matrix, dim=-2)
    return cops.complex_torch_sum(cauchy_matrix, dim=-2)
class SSKernelNPLR(OptimModule):
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        B = view_as_complex(self.B)
        C = view_as_complex(self.C)
        w = view_as_complex(self.w)  # (..., N)
        # z = view_as_complex(self.z) # (..., L)

        # TODO adjust based on rate times normal max length
        if L == self.L:
            nodes = view_as_complex(self.nodes)
            z = view_as_complex(self.z)  # (..., L)
        else:
            nodes, z = self._nodes(L, dtype=w.dtype, device=w.device)
        ...
        # w = w * dt.unsqueeze(-1)  # (... N)  #Xiaohui,** first complex operation failed.

        w = cops.complex_array_scalar_multiplication(w, dt.unsqueeze(-1))
      
        # Incorporate B and C batch dimensions
        #v = B.unsqueeze(-3) * C.unsqueeze(-2).conj()  # (..., 2, 2, N)
        
        v = cops.complex_multiple(cops.complex_unsqueeze(B,-3), cops.complex_conj(cops.complex_unsqueeze(C, -2)))
       ...
       # z = z[..., None, None, :]  # (..., 1, 1, L)
        ...
        z_r = z_r[..., None, None, :] 
        z_ig = z_ig[..., None, None, :]
        z = torch.complex(z_r, z_ig)

        # Calculate resolvent at nodes
        #if not self.keops and has_cauchy_extension:
        #    print ("========go to cauchy_mult===========")
         #   r = cauchy_mult(v, z, w)
            #r = cauchy_mult(v, z, w, symmetric=True)
        #else:
        #    print ("========go to cauchy_conj===========")
        #    r = cauchy_conj(v, z, w)
        
        r = cauchy_naive(v, z, w) #xiaohui added
        
        #r = r * dt[..., None, None, None]  # (..., 1+r, 1+r, L)
        r = cops.complex_array_scalar_multiplication(r , dt[..., None, None, None])  # (..., 1+r, 1+r, L)

    
        # Low-rank Woodbury correction
        if self.rank == 1:
            k_f = self.low_rank_woodbury_correction_rank_1(r)
        elif self.rank == 2:
        ...
        #k_f = k_f * 2 / (1 + nodes)
        k_f = cops.complex_division(cops.complex_scalar_multiple(k_f, 2), cops.complex_scalar_add (nodes, 1))
    ...
    def low_rank_woodbury_correction_rank_1(self, r):

        r_r = torch.real(r)
        r_ig = torch.imag(r)

        r1_r = r_r[..., :-1, :-1, :]  # train auc = 0.7333333333333334
        r1_ig = r_ig[..., :-1, :-1, :]
        r1 = torch.complex(r1_r, r1_ig)

        r2_r = r_r[..., :-1, -1:, :]
        r2_ig = r_ig[..., :-1, -1:, :]
        r2 = torch.complex(r2_r, r2_ig)

        r3_r = r_r[..., -1:, :-1, :]
        r3_ig = r_ig[..., -1:, :-1, :]
        r3 = torch.complex(r3_r, r3_ig)

        r4_r = r_r[..., -1:, -1:, :]
        r4_ig = r_ig[..., -1:, -1:, :]
        r4 = torch.complex(r4_r, r4_ig)


        #k_f = r[..., :-1, :-1, :] - r[..., :-1, -1:, :] * r[..., -1:, :-1, :] / (   #training auc= 0.711111111111111
        #         1 + r[..., -1:, -1:, :]
        #     )

        #k_f = r1 -  r2 * r3 / (1 + r4)   #training auc= 0.711111111111111
        k_f = cops.complex_minus(r1,                   # training auc = 0.7333333333333334
                                cops.complex_division(
                                       cops.complex_multiple(r2, r3), 
                                       cops.complex_scalar_add(r4, 1))
                               )
    ...
    def double_length(self):
        ...
        self.L *= 2
        dtype = view_as_complex(self.w).dtype
    ...
    def _setup(self, setup_C=True):
        w = _conj(view_as_complex(self.w))
        B = _conj(view_as_complex(self.B))
        C = _conj(view_as_complex(self.C))
        C = C.conj()
        p = B[..., -1, :]
        q = C[..., -1, :]
        B = B[..., 0, :]
...
class S4(nn.Module):
    ...
    def forward(self, u, state=None, cache=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        #y_f = k_f * u_f
        y_f = cops.complex_multiple(k_f, u_f)
