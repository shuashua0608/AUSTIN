...
class S4_MedIA_AST(nn.Module):
    ...
    def __init__(
        ...
        self.final_norm = nn.LayerNorm(1920)
        ...
        # Linear decoder
        self.decoder = nn.Linear(d_model+1536, d_output)
        self.uncertainty_head = nn.Linear(d_model+1536, 1)
        
        self.softmax = nn.Softmax(dim=1)
        self.softplus = nn.Softplus() #

    def forward(self, x, x_s, x_a, phrase='train'):
        """
        ...
        sigma=self.softplus(self.uncertainty_head(x))
        if phrase=='eval':
            return pred_score, sigma
        return pred_score, pred_logit, sigma, res
