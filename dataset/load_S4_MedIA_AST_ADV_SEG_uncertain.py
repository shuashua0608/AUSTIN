...
def load_all(labelid, roundid, video_root, audio_root, spec_root, video_list, tri_dic):
    ...
    for line in video_list:
        ...
        for clip in img_lists:
            ...
            wavs_first.append(os.path.join(audio_path, clip[:-4]+'.wav'))
...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.label == 1:
            ...
            fbank = self._wav2fbank(path_first[1]).squeeze(0)
            ...
            fbank_adv = self._wav2fbank(path_first[4]).squeeze(0)
            fbank_adv = (fbank + 4.26) / (4.57 * 2)
...
def main():
    ...
    train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset = LoadData(arg_root, audio_root, spec_path, 0, totallist, tri_dic,
                                                                             arg_batchsize_train,
                                                                             arg_batchsize_eval)
    count = 0
