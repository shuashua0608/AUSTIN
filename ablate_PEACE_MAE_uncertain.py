...
import dataset.load_S4_MedIA_PEACE_TRI as dataload
#from dataset import load_S4_MedIA_PEACE as dataload
from model import MAE_PEACE_uncertain as my_models
...
def train(gradient_scaler, train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (img_first, spec_first, audio_first, target_first, triage_first, index) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            pred_score, pred_logit, sigma = model(imgvar, fea, astvar, phrase='train')
            loss = criterion(pred_logit, target_var, triage_var, sigma)
            # print(pred_logit)
            # print(pred_score)
...
def validate(val_loader, model, val_dataset):
    ...
    index_vector = []
    with torch.no_grad():
        for i, (img_first, spec_first, audio_first, target_first, triage_first, index) in enumerate(val_loader):
            ...
            # compute output
            ''' model & full_model'''
            pred_score, sigma = model(imgvar, fea, astvar, phrase='eval')
