...
import dataset.load_S4_MedIA_PEACE_ADV_SEG_TRI as dataload
import model.S4_MedIA_PEACE_ADV_A5_driver as my_models
...
root = "."
wi = [2.0, 1.0]   # [1.25, 1.0]
postfix = "-m3-PEACE-grad-UNCERTAIN-Sigma-FULL-LR-4-AFTER-4"  #L200 means 200*2 frames, I original use 400*2 frames
...
# postfix = "-proposed-PEACE-V%d-32-L%d-R%d-E%d-ADAMW-0296-MOTION-STEP-1E-5"%(args.version,args.lamb,args.lr_ratio,args.epochs)
...
# class UncertaintyLoss(nn.Module):
#     def __init__(self, alpha=1.0, lambda_weight=0.1, epsilon=1e-6):
#         super().__init__()
#         self.alpha = alpha
#         self.lambda_weight = lambda_weight
#         self.epsilon = epsilon
...
#     def forward(self, y_hat, y_mri, y_triage, sigma):
#         # Compute weight for triage agreement
#         w = torch.exp(-self.alpha * torch.abs(y_mri - y_triage))
#         # Main uncertainty loss
#         print(y_hat.shape, y_mri.shape, y_triage.shape)
#         loss = ((y_hat - y_mri) ** 2) / (2 * sigma**2) + w * torch.log(sigma + self.epsilon)

#         # Auxiliary triage label loss
#         loss += self.lambda_weight * (y_hat - y_triage) ** 2  
#         return loss.mean()
...
def main():
    ...
    audio_root = '../stroke_data/fix_len_aud/seg_feat/'
    ...
    dis = my_models.Discriminator(n_downsampling=1, ndf=60)
    ...
    for param in model.encoder.audio_model.layer4.parameters():
        param.requires_grad = True
    for param in model.encoder.video_model.layer4.parameters():
        param.requires_grad = True
...
def train(gradient_scaler, train_loader, model, dis, criterion1, criterion2, optimizer1, optimizer2, epoch):
    ...
    for i, (input_first, spec_img, fbank_cookie, target_first, triage_first, index, input_adv, spec_adv, fbank_adv, target_adv) in enumerate(train_loader):
        # print(target_first,triage_first)
        ...
        with torch.cuda.amp.autocast():
            pred_score, pred_logit, sigma, fea = model(imgvar, specvar, cookievar, phrase='train')
            ...
            # print('train:', pred_logit.shape, target_var.shape, triage_var.shape)
            uncertainty_loss = criterion1(pred_logit, target_var, triage_var, sigma)
            model_loss = fake_dis_loss + uncertainty_loss*args.lamb
...
def validate(val_loader, model, val_dataset):
    ...
    with torch.no_grad():
        for i, (input_first, spec_img, audio_first, target_first, triage_first, index) in enumerate(val_loader):
            ...
            # compute output
            ''' model & full_model'''
            pred_score, sigma = model(imgvar, specvar, astvar, phrase='eval')
        ...
        #print(index_vector.shape, type(index_vector)) #torch.Size([252]) <class 'torch.Tensor'>
        index_matrix = []
